...
**Password mode**
- **Create**: generate a random 16-byte salt → one PBKDF2-HMAC-SHA-256 run at `PBKDF2_ITERATIONS` (600,000) producing 512 bits (`deriveKeyAndVerifier()`): the first 256 bits become the AES-GCM encryption key (imported non-extractable), the last 256 bits become the **verifier** — a value the server can check a password against without it being invertible into the actual encryption key (domain separation comes from the split itself). Encrypt as above. The share link is just `https://<host>/s/{id}` (no fragment — there's no key to embed). The salt, iteration count, and verifier are sent to the API as `kdf: { salt, iterations, verifier }` and stored server-side; none of them are secret — they're just parameters the recipient needs to re-derive the same key/verifier from the password. The single derivation means the creator/recipient pays exactly the same PBKDF2 cost an offline attacker pays per guess — no wasted second run. `CreateSecret.vue` also offers a "Suggest a password" button (`generateSecurePassword()`, CSPRNG-based) for users who don't want to pick their own.
